/**
 * Stage 4: Officer Dashboard Controller
 */
...
function updateMetrics(cases) {
  ...
  const pending = cases.filter(c => c.status === 'SUBMITTED' || c.status === 'VERIFICATION_PENDING').length;
...
function renderCaseTable(cases) {
  ...
  tbody.innerHTML = cases.map(c => {
    const citizenName = c.citizen ? c.citizen.fullName : 'N/A';
    const citizenPhone = c.citizen ? c.citizen.phone : 'N/A';
    ...
    return `
      <tr>
        ...
        <td>
          ...
          <div style="font-size:0.78rem; color:var(--text-dim);">${citizenPhone}</div>
        </td>
        <td>${c.category}</td>
